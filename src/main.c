...
#define TASK_DISPOSITIVO_STACK_SIZE                (1024*6/sizeof(portSTACK_TYPE))
#define TASK_DISPOSITIVO_STACK_PRIORITY            (tskIDLE_PRIORITY)
...
static void task_oled(void *pvParameters) {
	...
	for (;;)
	{
		if (xSemaphoreTake(xSemaphore, 1000)) {
			pin_toggle(TRIGGER_PIO, TRIGGER_PIO_PIN_MASK);
		}
	}
}
...
static void task_distancia(void *pvParameters) {
	...
	for (;;) {
		...
		{
			double distancia = (double)(n * 340) * 100 / (2 * 8600);
			sprintf(string, "%f", distancia);
...
static void BUT_init(void) {
	...
	pio_set_output(TRIGGER_PIO, TRIGGER_PIO_PIN_MASK, 0, 0, 0);
